...
def Plot_MCEP(dD_H2O):
    ...
    a13= 0.9471 # Mixing 0.9471; no mixing 0.9467 # alpha13C, averaged across all spikes
    ...
    gDDp=0.97 # 0.985 # Primary clumping factor gamma DD +/- e
    g13CDs=0.997 # 0.996 # Secondary clumping factor gamma 13CD +/- e
    gDDs=0.995 # 1.000 # Secondary clumping factor gamma DD +/- e
    # Errors
    eslope=6e-05 # mixing 6e-5; no mixing 0.0000823
    eintercept=0.0899722 # mixing 0.0899722; no mixing 0.0898757
    edD_MeOH=0.246 
    ed13C_MeOH=0.017 
    edD_H2O=0.0
    eD13CD=0.352
    ...
    ea13= 0.0052 # Mixing 0.0052 ;No mixing 0.0085 # Stdev of all alpha13C
...
def quick_plot(ax,y):
    ax.errorbar(MeOH['dD_H2O'].iloc[0], iso50[y],xerr=MeOH['dse_H2O'].iloc[0],yerr=0,
                markersize=16,label='Methylotrophic (' +r'$r$ ='+str(mixing_ratio[i])+')', fmt='o', 
                markerfacecolor=a, markeredgecolor='black',markeredgewidth=2.5,ecolor='black',elinewidth=2.5)
    ax.errorbar(MeOH_3000['dD_H2O'].iloc[0],iso3000[y],xerr=MeOH_3000['dse_H2O'].iloc[0],yerr=0,
                    markersize=16,fmt='o', 
                    markerfacecolor=a, markeredgecolor='black',markeredgewidth=2.5,ecolor='black',elinewidth=2.5)
    ax.errorbar(MeOH_8000['dD_H2O'].iloc[0],iso8000[y],xerr=MeOH_8000['dse_H2O'].iloc[0],yerr=0,
                    markersize=16,fmt='o', 
                    markerfacecolor=a, markeredgecolor='black',markeredgewidth=2.5,ecolor='black',elinewidth=2.5)
...
def quick_plot_star(ax,y):
    ax.errorbar(MeOH['dD_H2O'].iloc[0], iso50[y],xerr=MeOH['dse_H2O'].iloc[0],yerr=0.0,
                markersize=40,label='Methylotrophic (' +r'$r$ ='+str(mixing_ratio[i])+')', fmt='*', 
...
def quick_plot_reg(ax):
    ax.errorbar(MeOH['dD_H2O'].iloc[0]+1000, iso50[1]+1000,xerr=MeOH['dse_H2O'].iloc[0],yerr=0,
                markersize=32,label='Methylotrophic (' +r'$r$ ='+str(mixing_ratio[i])+')', fmt='*', 
                markerfacecolor='blue', markeredgecolor='black',markeredgewidth=2.5,ecolor='black',elinewidth=2.5)
    ax.errorbar(MeOH_3000['dD_H2O'].iloc[0]+1000,iso3000[1]+1000,xerr=MeOH_3000['dse_H2O'].iloc[0],yerr=0,
                    markersize=32,fmt='*', 
                    markerfacecolor='blue', markeredgecolor='black',markeredgewidth=2.5,ecolor='black',elinewidth=2.5)
    ax.errorbar(MeOH_8000['dD_H2O'].iloc[0]+1000,iso8000[1]+1000,xerr=MeOH_8000['dse_H2O'].iloc[0],yerr=0,
                    markersize=32,fmt='*', 
                    markerfacecolor='blue', markeredgecolor='black',markeredgewidth=2.5,ecolor='black',elinewidth=2.5)
...
dD=[MeOH['dD_H2O'].iloc[0],MeOH_3000['dD_H2O'].iloc[0],MeOH_8000['dD_H2O'].iloc[0]]
# Mix ratio
mixing_ratio=[1.0,0.99,0.985,0.98,0.975]
...
regMeOH,ax_regMeOH=plt.subplots(figsize=(12,12))
# quick_plot(ax_frac,'dD_CH4')
...
# ax_frac.set_ylim([-140,-40])
ax_frac.yaxis.set_minor_locator(MultipleLocator(20))
# quick_plot(ax_C_dD,'d13C_CH4')
...
# quick_plot(ax_CD_dD,'D13CH3D')
for i in range(len(dD_H2O_m)):
     if dD_H2O_m[i]==-50 or dD_H2O_m[i]==300:
          print(dD_H2O_m[i])
          print(p1[i,2])
...
ax_CD_dD.set_ylim([-21,0.5])
ax_CD_dD.yaxis.set_minor_locator(MultipleLocator(0.5))
# ax_CD_dD.set_ylim([-7.5,-2.5])
#ax_CD_dD.legend(fontsize=25)
# quick_plot(ax_DD_dD,'D12CH2D2')
...
ax_DD_dD.yaxis.set_minor_locator(MultipleLocator(10))
# ax_DD_dD.set_ylim([-40,-60])
ref=np.linspace(500,9500,100)
MeOH_ref=0.0798*ref+640.373
ax_regMeOH.plot(ref, MeOH_ref, '--', linewidth=5.0, color='black', label=r'$\delta$D$_{\rm CH_4}$+1000=0.0798($\delta$D$_{\rm H_2O}$+1000)+640')
ax_regMeOH.set_ylabel(r'$\delta D_{\rm CH_4}$'+' + 1000 (\u2030)',fontdict=font_labels)
ax_regMeOH.set_ylim([550,1490])
ax_regMeOH.yaxis.set_minor_locator(MultipleLocator(40))

for i in range(len(mixing_ratio)):
    iso50=calc_acet(np.mean(H2CO2['d13C_CH4']),np.mean(MeOH['d13C_CH4']),
                    np.mean(H2CO2['dD_CH4']),np.mean(MeOH['dD_CH4']),
                    np.mean(d13CH3D_H2CO2_50),np.mean(d13CH3D_target_50),
                    np.mean(d12CH2D2_H2CO2_50),np.mean(d12CH2D2_target_50),mixing_ratio[i])
    iso3000=calc_acet(np.mean(H2CO2_3000['d13C_CH4']),np.mean(MeOH_3000['d13C_CH4']),
                    np.mean(H2CO2_3000['dD_CH4']),np.mean(MeOH_3000['dD_CH4']),
    ...
    iso8000=calc_acet(np.mean(H2CO2_8000['d13C_CH4']),np.mean(MeOH_8000['d13C_CH4']),
                    np.mean(H2CO2_8000['dD_CH4']),np.mean(MeOH_8000['dD_CH4']),
    ...
    a=c[i]
    if mixing_ratio[i]!=0.985:
        quick_plot(ax_frac,1)
        quick_plot(ax_C_dD,0)
        quick_plot(ax_CD_dD,2)
        quick_plot(ax_DD_dD,3)
    if mixing_ratio[i]==0.985:
        quick_plot_star(ax_frac,1)
        quick_plot_star(ax_C_dD,0)
        quick_plot_star(ax_CD_dD,2)
        quick_plot_star(ax_DD_dD,3)
        quick_plot_reg(ax_regMeOH)
        ax_regMeOH.legend(fontsize=24)
        print(iso50)
        print(iso3000)
        print(iso8000) 
ax_C_dD.legend(fontsize=25)
# frac.savefig('MeOH_model_H.pdf', bbox_inches='tight')
# C_dD.savefig('MeOH_model_C.pdf', bbox_inches='tight')
# CD_dD.savefig('MeOH_model_CD.pdf', bbox_inches='tight')
# DD_dD.savefig('MeOH_model_DD.pdf', bbox_inches='tight')
# regMeOH.savefig('regMeOH.pdf', bbox_inches='tight')
